<template>
  <div>
    <div class="searchPanel">
      <el-row align="middle">
        <el-col>
          <el-button @click="addingABook" type="success" style="margin-right: 2%">add a new patron account</el-button>
        </el-col>

      </el-row>

      <div>
        <admin_patron-data_table style="margin: 0 auto;" v-model="tableData"></admin_patron-data_table>
      </div>

    </div>
    <div>
      <admin_patron-adding_popout-window v-model="userAddingModel"></admin_patron-adding_popout-window>
    </div>
  </div>
...
const QueryAllUser = async () => {
  ...
  axios({
    method: "GET",
    url: serverPath + "admin/listPatron"
  }).then(res => {
    tempResult = res.data;
