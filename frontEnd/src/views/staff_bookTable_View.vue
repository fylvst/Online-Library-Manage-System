<template>
  <div>
    <div class="searchPanel">
      <el-row align="middle">
        <el-col>
          <el-button @click="addingABook" type="success" style="margin-right: 2%">add a new book</el-button>
          <el-input
              v-model="inputValue"
      ...
      <div>
        <book-data-table-with-pagination style="margin: 0 auto;" v-model="tableData"></book-data-table-with-pagination>
      </div>

    </div>
    <div>
      <book-adding-window v-model="bookAddingModel"></book-adding-window>
    </div>
  </div>
    ...
    (newval, oldval) => {
      if (bookAddingModel.value.newData) {
        ...
        axios({
          ...
          let addBook = tempResult.book[0];
          tableData.value.reverse();
          rawData.value.reverse();
          tableData.value.push(addBook);
          rawData.value.push(addBook);
          tableData.value.reverse();
          rawData.value.reverse();
        });
