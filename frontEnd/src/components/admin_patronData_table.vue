  ...
  <div class="container">
    <div class="tablePart">
      ...
      >
        <el-table-column type="expand">
          <template #default="props">
            ...
            <div style="margin:0  5%">
              <p m="t-0 b-2">age: {{ props.row.age }}</p>
        ...
        <el-table-column prop="pid" align="center" label="id"/>
    ...
    >
    <span>
      Are you sure you want to delete patron <strong style="color: red"> {{ deleteInfo.userName }}</strong>
    </span>
      <template #footer>
...
const deleteUser = (scope: any) => {
  ...
  deleteInfo.value.pid = scope.row.pid;
...
const submitDelete = () => {
  axios({
    method: "POST",
    url: serverPath + "admin/delPatron",
    params: {
      pid: deleteInfo.value.pid,
    }
  }).then(res => {
    if (res.data.code == 200) {
      ...
      for (i = 0; i < rawData.value.length; i++)
        if (rawData.value[i].pid == deleteInfo.value.pid)
          break;
...
const editUser = (scope: any) => {
  axios({
    ...
    editInfo.value.pid = scope.row.pid;
    editInfo.value.userType = "patron";
...
watch(()=>{return editInfo.value.succeedAdding},async (newVal, oldVal) => {
  if (editInfo.value.succeedAdding) {
    for (let i = 0; i < rawData.value.length; i++) {
      ...
      if (rawData.value[i].pid == editInfo.value.pid) {
       await axios({
         method: "POST",
         url: serverPath + "patron/getByPid",
         params: {
           pid: editInfo.value.pid
         }
       }).then(res => {
