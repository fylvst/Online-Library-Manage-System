<template>
  <div class="container">
    <div class="tablePart">
      ...
      >
        ...
        <el-table-column prop="author" align="center" label="author"/><el-table-column prop="location" align="center" label="location"/>


        <!--      <el-table-column prop="description" label="description" width="1200"/> -->
        <el-table-column align="center" label="Operations">
          <template #default="scope">
            <el-button link type="warning" size="small" @click="lendABook(scope)">Lend</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
  ...
  <staff_book-lending_popout-window :model-value="bookLendingData"></staff_book-lending_popout-window>
</template>
...
import BookLendingDetailView from "@/views/bookLendingDetailView.vue";
import BookLending_popoutWindow from "@/components/patron_bookLending_popoutWindow.vue";
import Staff_bookLending_popoutWindow from "@/components/staff_bookLending_popoutWindow.vue";
...
watch(bookLendingData.value, async (newVal, oldVal) => {
  if (bookLendingData.value.succeed == true) {
    ...
    //tempScope.value.row.available -= 1;
